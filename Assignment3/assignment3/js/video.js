...
window.onload = function() {
  ...
  // Adding Video player to container
  var container = document.querySelector("#video");

  var newVideo = "<video width= '"+video.width+"' "+" height= '"+video.height+"' "+" controls= '"+video.controls+"' />";
       for (var i = 0; i < video.source.length; i++) {
            newVideo+= "<source src = '" +video.source[i].src+ "' "+" type= '" +video.source[i].type+ "' />";
        }

  container.innerHTML = newVideo;
};
